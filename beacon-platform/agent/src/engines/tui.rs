...
fn handle_key(state: &mut AppState, code: KeyCode, mods: KeyModifiers) {
    ...
    match code {
        ...
        KeyCode::Char('2') => {
            state.tab_index = 1;
            state.current_view = View::Agents;
        }
        KeyCode::Char('3') => {
            state.tab_index = 2;
            state.current_view = View::Metrics;
        }
        KeyCode::Char('4') => {
            state.tab_index = 3;
        ...
        KeyCode::Char('5') => {
            state.tab_index = 4;
            state.current_view = View::Health;
        }
        KeyCode::Char('6') => {
            state.tab_index = 5;
            state.current_view = View::Network;
        }
        KeyCode::Char('7') => {
            state.tab_index = 6;
            state.current_view = View::Configuration;
        }
...
fn draw_header(f: &mut Frame, state: &AppState, area: Rect) {
    ...
    let title = Line::from(vec![
        ...
        Span::raw("  │  "),
        Span::styled(
            &state.agent_id[..16.min(state.agent_id.len())],
...
fn draw_dashboard(f: &mut Frame, state: &AppState, area: Rect) {
    ...
    let cpu_gauge = Gauge::default()
        ...
        .gauge_style(Style::default().fg(if cpu > 80.0 {
            Color::Red
        } else if cpu > 60.0 {
            Color::Yellow
        } else {
    ...
    let ram_gauge = Gauge::default()
        ...
        .gauge_style(Style::default().fg(if ram > 80.0 {
            Color::Red
        } else if ram > 60.0 {
            Color::Yellow
        } else {
    ...
    let items: Vec<ListItem> = state
        ...
        .map(|l| {
            ...
            let line = Line::from(vec![
                Span::styled(
                    format!("[{}] ", &l.timestamp[11..19]),
                    Style::default().fg(Color::DarkGray),
                ),
                Span::styled(format!("{:<8}", l.severity), Style::default().fg(sev_color)),
                Span::raw(" "),
                Span::styled(
                    l.message.chars().take(60).collect::<String>(),
                    Style::default().fg(Color::White),
                ),
            ]);
            ListItem::new(line)
...
fn draw_metrics(f: &mut Frame, state: &AppState, area: Rect) {
    ...
    .header(
        Row::new(["Metric", "Value", "Unit"]).style(
            Style::default()
                .fg(Color::White)
...
fn draw_health(f: &mut Frame, state: &AppState, area: Rect) {
    ...
    .header(
        Row::new(["Collector", "Status"]).style(
            Style::default()
                .fg(Color::Cyan)
                .add_modifier(Modifier::BOLD),
        ),
